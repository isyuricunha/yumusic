    ...
    <div className="w-full space-y-6 pb-8">
      ...
      <Tabs defaultValue="albums" className="w-full">
        <TabsList className="bg-muted p-1 rounded-md">
          <TabsTrigger value="albums" className="rounded-sm">Recent Albums</TabsTrigger>
          <TabsTrigger value="artists" className="rounded-sm">Artists</TabsTrigger>
        </TabsList>

        <TabsContent value="albums" className="mt-6 outline-none">
          {loadingAlbums ? (
            <div className="text-muted-foreground animate-pulse">Loading albums...</div>
          ) : (
        ...
        <TabsContent value="artists" className="mt-6 outline-none">
          {loadingArtists ? (
            <div className="text-muted-foreground animate-pulse">Loading artists...</div>
          ) : (
              ...
              {artists?.map((artist) => (
                <div key={artist.id} className="group flex flex-col items-center cursor-pointer space-y-3">
                  <Avatar className="w-28 h-28 sm:w-32 sm:h-32 shadow-md transition-transform duration-300 group-hover:scale-[1.05]">
                    {/* Often artist generic photos are pulled by getCoverUrl based on Artist coverArt prop */}
